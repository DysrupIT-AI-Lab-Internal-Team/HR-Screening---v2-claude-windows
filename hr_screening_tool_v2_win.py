...
def build_converter(use_gpu=False):
    ...
    - use_gpu=True  → NVIDIA CUDA or DirectML (faster for large batches)
    - use_gpu=False → CPU (faster for small batches, no warmup overhead)
    """
    if not PDF_SUPPORT:
        return None
    ...
    if use_gpu and CUDA_AVAILABLE:
        ...
        # DirectML uses the same 'auto' device in some Docling versions;
        # fall back to CPU if unsupported
        try:
...
def extract_pdf_text(filepath, converter):
    ...
    try:
        ...
        return f"[Docling extraction failed: {e}]", False
...
def run_batch_screening(folder, jd_path, role_name, force_all=False, force_file=None):
    ...
    use_gpu = gpu_available and len(files) > GPU_BATCH_THRESHOLD
    if CUDA_AVAILABLE:
        device_lbl = f"CUDA -- {_gpu_name}" if use_gpu else "CPU"
    elif DIRECTML_AVAILABLE:
        device_lbl = "DirectML -- AMD/Intel GPU" if use_gpu else "CPU"
    else:
        device_lbl = "CPU"
    workers = MAX_WORKERS if len(files) > 1 else 1
    print(f"  [*]  Device: {device_lbl}  |  Workers: {workers}  |  Resumes: {len(files)}")
    if use_gpu:
        print(f"       (GPU enabled -- batch size {len(files)} exceeds threshold of {GPU_BATCH_THRESHOLD})")
...
def main():
    ...
    gpu_info = ""
    if CUDA_AVAILABLE:
        gpu_info = f"  GPU: CUDA -- {_gpu_name}"
    elif DIRECTML_AVAILABLE:
        gpu_info = "  GPU: DirectML (AMD/Intel)"
    else:
    ...
    print("   PDF extraction: Docling (IBM) -- High-accuracy Markdown")
    print(f"  {gpu_info}")
    print("=" * 62)
